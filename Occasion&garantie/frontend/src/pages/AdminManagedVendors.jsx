...
export default function AdminManagedVendors() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      alert(t('admin.error') + ': ' + (err.response?.data?.error || err.message));
    }
    setCreating(false);
  };
  ...
  return (
    <section className="admin-dashboard">
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '10px' }}>
              <FiUser size={28} style={{ color: 'var(--primary)' }} /> {t('admin.vendorAccountsTitle')}
            </h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>
              {vendors.length} {vendors.length > 1 ? t('admin.vendorAccountsPlural') : t('admin.vendorAccount')}
            </p>
          </div>
            ...
            <form onSubmit={handleCreate}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px', marginBottom: '16px' }}>
                <div className="form-group">
                  ...
                  <input value={newVendor.full_name} onChange={e => setNewVendor({ ...newVendor, full_name: e.target.value })} className="form-control" placeholder={t('admin.fullNamePlaceholder')} required />
                </div>
                <div className="form-group">
                  <label>{t('admin.storeNameLabel')}</label>
                  <input value={newVendor.store_name} onChange={e => setNewVendor({ ...newVendor, store_name: e.target.value })} className="form-control" placeholder={t('admin.storeNamePlaceholder')} />
                </div>
                <div className="form-group">
                  <label>{t('admin.cityLabel')}</label>
                  <input value={newVendor.ville} onChange={e => setNewVendor({ ...newVendor, ville: e.target.value })} className="form-control" placeholder={t('admin.cityPlaceholder')} list="villes" />
                  <datalist id="villes">
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button type="button" className="btn btn-ghost" onClick={() => setShowForm(false)}>{t('admin.cancel')}</button>
              </div>
            </form>
          </div>
        )}
        ...
        {resetPwResult && (
          <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 'var(--radius)', padding: '20px', marginBottom: '24px', position: 'relative' }}>
            ...
            <h3 style={{ color: '#3b82f6', marginBottom: '8px' }}>{t('admin.passwordResetTitle')}</h3>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '12px' }}>{t('admin.newPasswordFor')} <strong>{resetPwResult.full_name}</strong> :</p>
            <div style={{ fontFamily: 'monospace', fontSize: '14px', background: 'rgba(0,0,0,0.05)', padding: '12px', borderRadius: '8px', lineHeight: '2' }}>
              <div><strong>{t('admin.emailLabel')}</strong> {resetPwResult.email} <button onClick={() => copy(resetPwResult.email)} className="btn btn-ghost" style={{ padding: '2px 6px', fontSize: '11px' }}><FiCopy size={12} /></button></div>
              <div><strong>{t('admin.passwordLabel')}</strong> <span style={{ color: '#3b82f6', fontWeight: 700 }}>{resetPwResult.password}</span> <button onClick={() => copy(resetPwResult.password)} className="btn btn-ghost" style={{ padding: '2px 6px', fontSize: '11px' }}><FiCopy size={12} /></button></div>
            </div>
          </div>
        )}
        ...
        {createdCreds && (
          <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 'var(--radius)', padding: '20px', marginBottom: '24px' }}>
            <h3 style={{ color: '#10b981', marginBottom: '8px' }}>{t('admin.accountCreatedTitle')}</h3>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '12px' }}>{t('admin.copyCredentials')}</p>
            <div style={{ fontFamily: 'monospace', fontSize: '14px', background: 'rgba(0,0,0,0.05)', padding: '12px', borderRadius: '8px', lineHeight: '2' }}>
              <div><strong>{t('admin.emailLabel')}</strong> {createdCreds.email} <button onClick={() => copy(createdCreds.email)} className="btn btn-ghost" style={{ padding: '2px 6px', fontSize: '11px' }}><FiCopy size={12} /></button></div>
              <div><strong>{t('admin.passwordLabel')}</strong> <span style={{ color: '#3b82f6', fontWeight: 700 }}>{createdCreds.password}</span> <button onClick={() => copy(createdCreds.password)} className="btn btn-ghost" style={{ padding: '2px 6px', fontSize: '11px' }}><FiCopy size={12} /></button></div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>{t('admin.keepCredentials')}</div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: '13px' }}>{t('admin.noVendorsHint')}</p>
          </div>
        ) : (
            ...
            <table className="admin-table">
              <thead>
                <tr>
                  ...
                  <th>{t('admin.thStore')}</th>
                  <th>{t('admin.thEmail')}</th>
                  <th>{t('admin.thCity')}</th>
                  <th>{t('admin.thCreated')}</th>
                  <th style={{ width: 80 }}>{t('admin.thActions')}</th>
                </tr>
              </thead>
              <tbody>
                {vendors.map(v => (
      ...
      <ConfirmModal
        ...
        title={t('admin.resetPwConfirmTitle')}
        message={t('admin.resetPwConfirmMessage', { name: resetPwTarget?.full_name || '' })}
        confirmText={t('admin.resetPwConfirmButton')}
        confirmColor="#3b82f6"
      ...
      <ConfirmModal
        ...
        title={t('admin.deleteVendorTitle')}
