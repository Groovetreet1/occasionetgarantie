  ...
  return (
    <section className="admin-dashboard">
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: 800 }}>{t('admin.creditPurchasesTitle')}</h1>
            <p style={{ color: 'var(--text-secondary)' }}>{purchases.length} {purchases.length > 1 ? t('admin.requestPlural') : t('admin.requestSingular')}</p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <Link to="/admin/credits" className="btn btn-primary" style={{ fontSize: 13 }}>{t('admin.creditsTab')}</Link>
        ...
        {loading ? (
          ...
          <div className="empty-state"><FiCreditCard size={48} /><p>{t('admin.noCreditPurchases')}</p></div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-muted)', textTransform: 'uppercase', fontSize: '11px', letterSpacing: '0.5px' }}>
                  ...
                  <th style={{ padding: '12px 8px', textAlign: 'left' }}>{t('admin.thCredits')}</th>
                ...
                {purchases.map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 8px' }}>
                      {p.status === 'confirme' ? (
                        <span style={{ color: 'var(--success)', display: 'flex', alignItems: 'center', gap: 4 }}><FiCheck size={14} /> {t('admin.confirmedStatus')}</span>
                      ) : p.status === 'rejete' ? (
                        <span title={p.rejection_reason} style={{ color: 'var(--error)', display: 'flex', alignItems: 'center', gap: 4, cursor: 'help' }}><FiX size={14} /> {t('admin.rejectedStatus')}</span>
                      ) : (
      ...
      {rejectModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', padding: '20px' }}
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', padding: '32px', maxWidth: '480px', width: '100%', border: '1px solid var(--border)' }}
            ...
            <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '8px' }}>{t('admin.rejectCreditsTitle')}</h2>
            ...
            <RejectForm
              ...
              defaultMsg={t('admin.defaultRejectMsg')}
            />
          </div>
        </div>
      )}
      ...
      <ConfirmModal
        ...
        title={t('admin.confirmPurchaseTitle')}
        message={t('admin.confirmPurchaseMessage')}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
        <button onClick={onCancel} className="btn btn-ghost" disabled={loading} style={{ padding: '10px 20px' }}>{t('admin.cancel')}</button>
