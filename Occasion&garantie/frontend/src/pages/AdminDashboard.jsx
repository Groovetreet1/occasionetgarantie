                ...
                {products.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td style={{ fontWeight: 600 }}>
                      {p.featured && <span className="admin-top">TOP</span>}
                      {p.name}
                    </td>
                    <td style={{ color: 'var(--primary)', fontWeight: 700 }}>{formatPrice(p.price)}</td>
                    <td style={{ color: 'var(--text-secondary)' }}>{p.category_name || '-'}</td>
                    <td><span style={{ padding: '2px 10px', borderRadius: '10px', background: 'var(--primary-light)', fontSize: '12px' }}>{stateLabels[p.state] || p.state}</span></td>
                    <td>{p.stock > 0 ? <span style={{ color: 'var(--success)' }}>✓ {p.stock}</span> : <span style={{ color: 'var(--error)' }}>Rupture</span>}</td>
                    <td>
