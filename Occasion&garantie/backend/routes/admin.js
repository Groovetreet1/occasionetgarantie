...
router.post('/seed', authenticate, adminOnly, async (req, res) => {
  try {
    ...
    for (const p of seedProducts) {
      ...
      const [existing] = await pool.query('SELECT id, image, price FROM products WHERE slug = ?', [slug]);
      if (existing.length > 0) {
        const updates = [];
        const params = [];
        if (!existing[0].image && image) { updates.push('image = ?'); params.push(image); console.log('  Image added:', name); }
        if (Number(existing[0].price) === 1) {
          updates.push('price = ?'); params.push(price);
          updates.push('old_price = ?'); params.push(old_price || null);
      ...
      await pool.query(
        ...
        [name, slug, description, price, old_price || null, category_id, brand, state, warranty, stock, JSON.stringify(seedSpecs), image || null]
      );
      created++;
    }
