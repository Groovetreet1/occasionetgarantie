...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    try {
      await send({
        to: process.env.CONTACT_EMAIL || 'contact@contact.occasionetgarantie.store',
        subject: `[Occasion & Garantie] Message de ${full_name}`,
        html: `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"></head><body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f8f9fc"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px"><tr><td align="center"><table role="presentation" width="100%" style="max-width:480px;background:#fff;border-radius:12px;padding:32px"><tr><td><h1 style="font-size:18px;color:#1e293b;margin:0 0 16px">Nouveau message de contact</h1><table cellpadding="8"><tr><td style="font-size:13px;color:#64748b;width:60px">Nom</td><td style="font-size:14px;font-weight:600;color:#1e293b">${full_name}</td></tr><tr><td style="font-size:13px;color:#64748b">Email</td><td style="font-size:14px;color:#f59e0b">${email}</td></tr></table><div style="margin-top:16px;padding:16px;background:#f0f1f5;border-radius:8px;font-size:14px;color:#1e293b;line-height:1.6">${message.replace(/\n/g, '<br>')}</div></td></tr></table></td></tr></table></body></html>`,
      });
    } catch (mailErr) {
